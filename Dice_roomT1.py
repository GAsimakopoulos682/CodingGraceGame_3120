...
def dice_room(player_info_arg):
    print_dice()
    """The Dice Room: roll the dice and accept your fate.

    Odd numbers trigger a trapdoor and kill the player.
    Even numbers allow the player to escape and win the game.

    Returns:
        "flee" if the player chooses not to roll the dice.
    """
    ...
    print("\nThe walls are patterned in black and white squares.")
    print("In the center of the room sits a massive stone dice.")
    print('Carved into the floor are the words:')
    print('"Roll the dice and accept your fate."')

    action = input("\nDo you roll the dice or leave? [roll | leave] > ").lower()

    if action == "leave":
        print("You decide not to tempt fate and return to the dungeon doors.")
        return "flee"

    elif action == "roll":
        print("\nYou roll the giant dice...")

        dice_roll = random.randint(1, 6)

        print("\nThe dice tumbles across the checkered floor...")
        print(f"You rolled a {dice_roll}!")
             
        print_dice_face(dice_roll)

        if dice_roll % 2 == 1:
            you_died("A trapdoor suddenly opens beneath you and you fall into darkness")

        else:
            you_won("The checkered wall slides open and you escape the dungeon")

    else:
        print("Not sure what you meant. Returning to the dungeon doors.")
        return "flee"
...
def start_new_adventure(player_info_arg):
    ...
    while True:
        ...
        else: print("Sorry, it's either 'red', 'blue', or 'green' as the "
